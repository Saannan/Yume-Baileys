...
const makeSocket = (config) => {
    var _a, _b;
    // Fast Response Optimization v2.1.15
    config.connectTimeoutMs = config.connectTimeoutMs || 20000; // Reduced from 60s to 20s
    config.keepAliveIntervalMs = config.keepAliveIntervalMs || 25000; // More frequent keepalive
    config.defaultQueryTimeoutMs = config.defaultQueryTimeoutMs || 30000; // Faster timeout
    ...
    const onUnexpectedError = (err, msg) => {
        ...
        // gently back off when encountering rate limits (429) - OPTIMIZED
        if (message.includes('429') || message.includes('rate limit')) {
            const wait = Math.min(5000, (config.backoffDelayMs || 1000)); // Reduced: 30s→5s max, 5s→1s default
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "ZZAMDEV1") => {
        if (pairKey) {
    ...
    return {
        ...
        reconnect: async () => {
            ...
            try {
                ...
                logger.error({ error }, 'error during reconnect');
                end(error);
            }
        },
        autoReconnect: () => {
            ev.on('connection.update', async ({ connection, lastDisconnect }) => {
                if (connection === 'close') {
                    const shouldReconnect = (lastDisconnect?.error)?.output?.statusCode !== Types_1.DisconnectReason.loggedOut;
                    if (shouldReconnect) {
