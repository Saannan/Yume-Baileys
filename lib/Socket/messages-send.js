...
const makeMessagesSocket = (config) => {
    const { logger, linkPreviewImageThumbnailWidth, generateHighQualityLinkPreview, options: axiosOptions, patchMessageBeforeSending, cachedGroupMetadata, rateLimiter, } = config;
    ...
    // Rate Limiter & Flood Protection (OPTIMIZED FOR SPEED)
    const messageRateCache = new Map();
    const defaultRateLimiter = {
        maxMessagesPerMinute: 120, // Increased: 120 pesan per menit (2x faster)
        delayBetweenMessages: 50, // Reduced: 50ms delay (20x faster dari 1000ms!)
        enabled: false // DISABLED by default untuk performa maksimal
    };
    const rateLimiterConfig = rateLimiter || defaultRateLimiter;

    const checkRateLimit = async (jid) => {
        if (!rateLimiterConfig.enabled) return;

        const now = Date.now();
        const userKey = (0, WABinary_1.jidNormalizedUser)(jid);
        const userRateData = messageRateCache.get(userKey) || {
            messages: [],
            lastMessageTime: 0
        };

        // Bersihkan pesan lama (lebih dari 1 menit)
        const oneMinuteAgo = now - 60000;
        userRateData.messages = userRateData.messages.filter(time => time > oneMinuteAgo);

        // Cek batas pesan per menit
        if (userRateData.messages.length >= rateLimiterConfig.maxMessagesPerMinute) {
            const oldestMessage = Math.min(...userRateData.messages);
            const waitTime = 60000 - (now - oldestMessage);
            if (waitTime > 0) {
                logger.warn(`Rate limit exceeded for ${userKey}, waiting ${waitTime}ms`);
                await (0, Utils_1.delay)(waitTime);
                return checkRateLimit(jid); // Rekursif untuk cek ulang setelah delay
            }
        }

        // Cek delay antar pesan
        const timeSinceLastMessage = now - userRateData.lastMessageTime;
        if (timeSinceLastMessage < rateLimiterConfig.delayBetweenMessages) {
            const waitTime = rateLimiterConfig.delayBetweenMessages - timeSinceLastMessage;
            logger.debug(`Applying delay for ${userKey}: ${waitTime}ms`);
            await (0, Utils_1.delay)(waitTime);
        }

        // Update data rate
        userRateData.messages.push(now);
        userRateData.lastMessageTime = Date.now();
        messageRateCache.set(userKey, userRateData);
    };
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        var _a;
        // Auto JID handling - normalize JID
        jid = (0, WABinary_1.jidNormalizedUser)(jid);
        ...
        await authState.keys.transaction(async () => {
            ...
            // Tambahkan label AI untuk pesan pribadi
            const isPrivate = !isGroup && !isNewsletter && !isStatus;
            const filterBotNode = (nodes) => nodes.filter(node => node.tag === 'bot');
            ...
            try {
                ...
                // Handle badmac and session errors
                if (error.message?.includes('badmac') || error.message?.includes('session')) {
                    logger.warn({ msgId, error: error.message }, 'Message send failed, attempting to refresh session');

                    // Try to re-establish sessions for all participants
                    try {
                        await assertSessions(allJids, true);
                        logger.debug({ msgId }, 'Session refreshed, retrying message send');
                        await sendNode(stanza);
                    } catch (retryError) {
                        logger.error({ msgId, error: retryError.message }, 'Failed to send message after session refresh');
                        throw retryError;
                    }
                } else {
                    throw error;
    ...
    return {
        ...
        getRateLimiterStatus: () => ({
            ...
            maxMessagesPerMinute: rateLimiterConfig.maxMessagesPerMinute,
            delayBetweenMessages: rateLimiterConfig.delayBetweenMessages,
            activeUsers: messageRateCache.size
        }),
        setRateLimiter: (config) => {
            if (config.maxMessagesPerMinute !== undefined) {
                rateLimiterConfig.maxMessagesPerMinute = config.maxMessagesPerMinute;
            }
            if (config.delayBetweenMessages !== undefined) {
                rateLimiterConfig.delayBetweenMessages = config.delayBetweenMessages;
            }
            if (config.enabled !== undefined) {
                rateLimiterConfig.enabled = config.enabled;
            }
            logger.info('Rate limiter updated:', rateLimiterConfig);
        },
        resetRateLimiter: (jid = null) => {
            if (jid) {
                const userKey = (0, WABinary_1.jidNormalizedUser)(jid);
                messageRateCache.delete(userKey);
                logger.debug(`Rate limiter reset for user: ${userKey}`);
            } else {
                messageRateCache.clear();
                logger.info('Rate limiter reset for all users');
            }
        },
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // Apply Rate Limiter & Flood Protection
            await checkRateLimit(jid);

            // Fake Reply Feature - Auto-inject fake quoted message (v2.1.14)
            ...
            if (options.quoted === undefined) {
                ...
                logger.debug('Auto-injected fake quoted message (v2.1.14)');
            }
            ...
            }
            {
                ...
                if (isAiMsg) {
                    // AI label and footer removed as per user request
                }

                // 💾 FITUR EKSKLUSIF @ajammm/baileys - AUTO BACKUP
                if (config.autoBackup && config.autoBackup.enabled) {
                    try {
                        const fs = require('fs').promises;
                        const path = require('path');
                        const backupDir = config.autoBackup.path || './backups';
                        ...
                        const filename = `msg_backup_${timestamp}.json`;
                        const filepath = path.join(backupDir, filename);

                        await fs.writeFile(filepath, JSON.stringify(backupData, null, 2));

                        // Hapus backup lama (simpan maksimal 1000 file)
                        const files = await fs.readdir(backupDir).catch(() => []);
                        const backupFiles = files.filter(f => f.startsWith('msg_backup_')).sort();

                        if (backupFiles.length > 1000) {
                            const filesToDelete = backupFiles.slice(0, backupFiles.length - 1000);
                            for (const file of filesToDelete) {
                                await fs.unlink(path.join(backupDir, file)).catch(() => {});
                            }
                        }

                        logger.debug(`💾 Message backed up: ${filename}`);
                    } catch (error) {
                        logger.warn('Auto backup failed:', error.message);
                    }
                }
                if (mediaHandle) {
        ...
        sendAlbumMessage: async (jid, medias, options = {}) => {
            // Apply Rate Limiter & Flood Protection for album
            await checkRateLimit(jid);

            const userJid = authState.creds.me.id;
            for (const media of medias) {
                if (!media.image && !media.video)
                    throw new TypeError(`medias[${medias.indexOf(media)}] must have image or video property`);
            }
            if (medias.length < 2)
                throw new RangeError("Minimum 2 media");
            const time = options.delay || 100; // OPTIMIZED: 500ms → 100ms (5x faster!)
